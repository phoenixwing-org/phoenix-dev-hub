...
> 日期：2026-08-02
> 范围：仅 `phoenix-dev-hub`；不修改 Phoenix Admin Host 或业务插件源码，不执行 DDL，不 push。
...
- [x] 在 `系统 → Dev Hub → Hub 设置` 增加独立 View。
...
- [ ] 打开“系统 → Dev Hub → Hub 设置”，确认暗色主题、布局和文案正常。
- [ ] 点击“打开 Hub 终端”，确认终端停留在 `phoenix-dev-hub` 目录且未自动执行命令。
