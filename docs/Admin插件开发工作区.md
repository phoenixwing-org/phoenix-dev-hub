...
仓库提供 [`config/sample/admin-plugins.sample.json`](../config/sample/admin-plugins.sample.json)。复制为
...
若任一插件处于已挂载、部分挂载或路径冲突状态，Hub 拒绝更换开发 Host。先完成开发卸载，
再修改 Host 根目录或服务 ID，避免遗留不可追踪的旧 symlink。
