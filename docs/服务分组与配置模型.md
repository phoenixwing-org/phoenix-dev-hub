...
编排和核验由独立的 [Admin 插件开发工作区](Admin插件开发工作区.md) 负责。
...
- **Service**：一条受控命令和独立进程生命周期，仍由 `PnhServiceManager` 管理。
...
仓库只归档去敏的 `config/sample/services.sample.json`，实际基线由使用者复制到 Git 忽略的
...
- 可归档示例：`config/sample/services.sample.json`；
...
- 整套导出：`phoenix-hub-config` version 2；
