...
describe("pnhProfileDatabaseConfirmation", () => {
  it("只为 release-validation 生成固定确认文本和回收责任提示", () => {
    expect(pnhProfileDatabaseConfirmation(status("release-validation"))).toEqual({
    ...
    expect(pnhProfileDatabaseConfirmation(status("development"))).toBeUndefined();
  });
});
