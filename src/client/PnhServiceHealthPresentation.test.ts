...
describe("PnhServiceHealthPresentation", () => {
  it("将启动宽限期内的暂时不可达显示为正在检查", () => {
    ...
    expect(pnhPresentedHealth(service)).toEqual({ state: "checking", label: "正在检查" });
    expect(pnhPresentedEndpoint(service, endpoint)).toEqual({
      state: "checking",
      label: "CHECKING",
    });
  });
  ...
  it("明确构建失败时不使用正在检查掩盖错误", () => {
    ...
    expect(pnhPresentedHealth(service)).toEqual({
      state: "unhealthy",
      label: "健康检查失败",
    });
  });
