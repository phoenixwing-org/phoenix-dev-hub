...
defineOptions({ name: "PnhAdminPluginPrimaryPanel" });
...
async function inspect(): Promise<void> {
  ...
  try {
    candidate.value = await hubApi.inspectAdminPlugin(directory.value.trim());
  } catch (error) {
    emit("error", error);
  } finally {
    busy.value = false;
...
async function add(): Promise<void> {
  ...
  busy.value = true;
  try {
    const added = await hubApi.addAdminPlugin(directory.value.trim());
    adding.value = false;
    directory.value = "";
    candidate.value = undefined;
...
<style scoped>
.plugin-primary { width: 100%; height: 100%; color: var(--pnw-workbench-text, var(--pnw-workbench-default-text, #0f172a)); }
.primary-count { min-width: 20px; display: inline-block; padding: 1px 5px; border-radius: 999px; background: rgba(148,163,184,.14); color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-size: 10px; text-align: center; }
.plugin-section { padding: 8px; }
.plugin-list-section { display: grid; gap: 8px; }
button { border: 1px solid var(--pnw-workbench-border, var(--pnw-workbench-default-border, #dbe3ed)); background: transparent; color: inherit; cursor: pointer; }
button:hover { background: var(--pnw-control-hover-bg, var(--pnw-workbench-default-hover-bg, rgba(59, 130, 246, .09))); }
button:disabled { cursor: not-allowed; opacity: .45; }
.primary-add { width: 28px; height: 26px; padding: 0; border-radius: 6px; font-size: 18px; line-height: 1; }
.add-card { display: grid; gap: 7px; padding: 9px; border: 1px solid var(--pnw-workbench-border, var(--pnw-workbench-default-border, #dbe3ed)); border-radius: 8px; background: var(--pnw-workbench-surface, var(--pnw-workbench-default-surface, #fff)); }
label { display: grid; gap: 4px; }
label span { color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-size: 10px; }
input { min-width: 0; box-sizing: border-box; border: 1px solid var(--pnw-workbench-border, var(--pnw-workbench-default-border, #dbe3ed)); border-radius: 6px; padding: 7px 8px; background: var(--pnw-control-bg, var(--pnw-workbench-default-control-bg, #fff)); color: inherit; outline: none; }
...
.candidate strong { font-size: 11px; }.candidate span,.candidate small { color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-size: 9px; }
.candidate .warning { color: #f59e0b; white-space: normal; }
.search { display: flex; align-items: center; gap: 5px; padding: 0 2px; }.search input { width: 100%; border-radius: 5px; padding: 6px; font-size: 10px; }.search span { color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); }
...
.plugin-name { overflow: hidden; text-overflow: ellipsis; white-space: nowrap; font-size: 11px; font-weight: 750; }.plugin-meta { color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-family: ui-monospace,monospace; font-size: 9px; }
.plugin-state { grid-column: 1 / -1; justify-self: start; padding: 1px 5px; border-radius: 999px; background: rgba(148,163,184,.12); color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-size: 9px; }.plugin-state[data-state="mounted"] { background: rgba(34,197,94,.12); color: #22c55e; }.plugin-state[data-state="partial"],.plugin-state[data-state="conflict"],.plugin-state[data-state="unavailable"] { background: rgba(245,158,11,.13); color: #f59e0b; }
.empty-list { padding: 20px 8px; color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-size: 10px; text-align: center; }
.hint { margin: 0; padding: 9px 10px; color: var(--pnw-workbench-muted, var(--pnw-workbench-default-muted, #64748b)); font-size: 9px; line-height: 1.55; }
</style>
