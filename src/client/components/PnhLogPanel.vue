...
defineOptions({ name: "PnhLogPanel" });
...
async function copyLogs(): Promise<void> {
  try {
    ...
    await navigator.clipboard.writeText(pnhRuntimeLogClipboardText(props.entries));
...
function logTone(entry: LogEntry): "default" | "warning" | "error" {
  ...
  const text = pnhDisplayLogText(entry.text);
  ...
  <section class="log-panel">
    ...
    <ol v-if="entries.length">
      ...
      >
        ...
        <b>{{ entry.stream }}</b>
        <span>{{ pnhDisplayLogText(entry.text) }}</span>
      </li>
    </ol>
