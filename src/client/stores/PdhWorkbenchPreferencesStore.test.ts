...
import { usePdhWorkbenchPreferencesStore } from "./PdhWorkbenchPreferencesStore";
...
describe("PdhWorkbenchPreferencesStore", () => {
  ...
  it("恢复用户主题并在修改后持久化", async () => {
    storage.setItem("phoenix-dev-hub.workbench.v1", JSON.stringify({ colorScheme: "light" }));
    const store = usePdhWorkbenchPreferencesStore();
    expect(store.colorScheme).toBe("light");

    store.colorScheme = "dark";
    await nextTick();
    expect(JSON.parse(storage.getItem("phoenix-dev-hub.workbench.v1") ?? "{}")).toMatchObject({
      colorScheme: "dark",
    });
  });

  it("恢复并持久化服务搜索与排序偏好", async () => {
    storage.setItem("phoenix-dev-hub.workbench.v1", JSON.stringify({
