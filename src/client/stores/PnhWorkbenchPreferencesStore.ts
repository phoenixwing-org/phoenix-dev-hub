...
const PREFERENCES_KEY = "phoenix-hub.workbench.v1";
export type PnhServiceSortMode = "configured" | "name" | "status" | "port";

interface PnhStoredPreferences extends Partial<PnwWorkbenchDisplayPreferences> {
  readonly serviceSearchQuery?: string;
  readonly serviceSortMode?: PnhServiceSortMode;
  readonly collapsedServiceSeriesIds?: readonly string[];
  readonly collapsedServiceProfileIds?: readonly string[];
}

function isServiceSortMode(value: unknown): value is PnhServiceSortMode {
...
function loadPreferences(): {
  ...
  try {
    ...
    if (stored) {
      const parsed = JSON.parse(stored) as PnhStoredPreferences;
      return {
...
/** 持有并持久化 Hub 工作台显示偏好；首次使用跟随系统。 */
export const usePnhWorkbenchPreferencesStore = defineStore(
  "pnhWorkbenchPreferences",
  () => {
    ...
    const serviceSortMode = ref<PnhServiceSortMode>(saved.serviceSortMode);
