...
export interface PdhBuiltinImportPlan {
  readonly definitions: readonly ServiceDefinition[];
}

export interface PdhBuiltinSeriesImportPlan {
...
function configError(message: string, statusCode = 500): never {
  throw new DevHubError("INVALID_BUILTIN_SERVICE_CONFIG", message, statusCode);
}
...
export class PdhBuiltinServiceConfigStore {
  ...
  update(serviceId: string, value: unknown): ServiceDefinition {
    ...
    if (!current) throw new DevHubError("BUILTIN_SERVICE_NOT_FOUND", `未知内置服务：${serviceId}`, 404);
  ...
  updateSeries(seriesId: string, value: unknown): readonly ServiceDefinition[] {
    ...
    if (existingIndex < 0) throw new DevHubError("BUILTIN_SERIES_NOT_FOUND", `未知 Series：${seriesId}`, 404);
  ...
  restore(serviceId: string): ServiceDefinition {
    ...
    if (!definition) throw new DevHubError("BUILTIN_SERVICE_NOT_FOUND", `未知内置服务：${serviceId}`, 404);
  ...
  prepareImport(values: readonly unknown[]): PdhBuiltinImportPlan {
  ...
  commitImport(plan: PdhBuiltinImportPlan): readonly ServiceDefinition[] {
  ...
  commitSeriesImport(plan: PdhBuiltinSeriesImportPlan): readonly ServiceDefinition[] {
