...
import { PdhPostgresPreflight, type PdhPostgresConnector } from "./PdhPostgresPreflight.js";
...
describe("PdhPostgresPreflight", () => {
  it("只读确认隔离数据库存在且不把凭据写入证据", async () => {
    const query = vi.fn().mockResolvedValue({
      rows: [{ serverVersion: "17.5", exists: true }],
    });
    const end = vi.fn().mockResolvedValue(undefined);
    const connector: PdhPostgresConnector = vi.fn().mockResolvedValue({ query, end });
    const definition = releaseDefinition();
    const preflight = new PdhPostgresPreflight(connector, {});
  ...
  it("数据库缺失时阻断 spawn，并使用稳定错误码而不泄露密码", async () => {
    const connector: PdhPostgresConnector = vi.fn().mockResolvedValue({
  ...
  it("连接失败只暴露错误码，不暴露底层连接文本", async () => {
    ...
    const connector: PdhPostgresConnector = vi.fn().mockRejectedValue(failure);
    const definition = releaseDefinition();
    const preflight = new PdhPostgresPreflight(connector, {});
  ...
  it("数据库存在但缺 Host/Pah 基线关系时标记 uninitialized 并在 spawn 前阻断", async () => {
    ...
    const connector: PdhPostgresConnector = vi.fn()
    ...
    const blockingConnector: PdhPostgresConnector = vi.fn()
      .mockResolvedValueOnce({ query: maintenanceQuery, end: vi.fn().mockResolvedValue(undefined) })
      .mockResolvedValueOnce({ query: targetQuery, end: vi.fn().mockResolvedValue(undefined) });
    const blockingPreflight = new PdhPostgresPreflight(blockingConnector, {});
  ...
  it("缺少显式用户名变量时 fail-closed，不回退 USER 或 postgres", async () => {
    ...
    const preflight = new PdhPostgresPreflight(connector, { USER: "ambient-user" });
  ...
  it("仅通过显式确认创建 allowlist 中原先不存在的本机验收库并记录回收责任", async () => {
    const root = mkdtempSync(path.join(tmpdir(), "pdh-postgres-create-"));
    let exists = false;
    const connector: PdhPostgresConnector = vi.fn().mockImplementation(async () => ({
