...
function fixture(localDependency = false, wingPeer?: string) {
  const root = mkdtempSync(path.join(tmpdir(), "pdh-profile-assembly-"));
  roots.push(root);
...
describe("PdhProfileAssembly", () => {
  ...
  it("拒绝本地依赖协议并清理未完成 assembly", async () => {
    ...
    await expect(new PdhProfileAssembly().prepare(value.definitions)).rejects.toMatchObject({
  ...
  it("包 SHA 或既有无证据目录变化时 fail-closed", async () => {
    ...
    await expect(new PdhProfileAssembly().prepare(definitions)).rejects.toMatchObject({
      code: "PROFILE_PREFLIGHT_FAILED",
  ...
  it("拒绝串库和不满足业务包 Host peer 的 Registry 精确版本", async () => {
    ...
    const crossedDatabase = databaseValue.definitions.map((definition) => definition.serviceRole === "api"
      ...
      : definition);
    await expect(new PdhProfileAssembly().prepare(crossedDatabase)).rejects.toMatchObject({
      code: "PROFILE_PREFLIGHT_FAILED",
      message: expect.stringContaining("数据库环境变量"),
    });

    const peerValue = fixture(false, ">=0.7.0 <0.8.0");
    await expect(new PdhProfileAssembly().prepare(peerValue.definitions)).rejects.toMatchObject({
      code: "PROFILE_PREFLIGHT_FAILED",
