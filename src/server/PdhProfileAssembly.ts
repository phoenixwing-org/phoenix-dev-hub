...
function readJson(filePath: string): Record<string, unknown> {
  try {
    ...
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `JSON 证据无效：${filePath}`, 409);
  }
}

function safeRelative(value: unknown, label: string): string {
  if (typeof value !== "string" || !value || value.includes("\\")) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `${label} 不是安全相对路径`, 409);
  }
  ...
  if (
    ...
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `${label} 不是安全相对路径：${value}`, 409);
  }
  return normalized;
}
...
function walkFiles(root: string, relative = ""): string[] {
  ...
  for (const entry of readdirSync(current, { withFileTypes: true }).sort((left, right) => left.name.localeCompare(right.name))) {
    ...
    if (stat.isSymbolicLink()) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `装配输入包含符号链接：${absolute}`, 409);
    }
    if (stat.isDirectory()) files.push(...walkFiles(root, child));
    else if (stat.isFile()) files.push(child.replaceAll("\\", "/"));
    else throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `装配输入包含特殊文件：${absolute}`, 409);
  }
  return files;
}
...
async function validateGitInput(label: string, root: string, expectedCommit: string): Promise<string> {
  ...
  if (dirty.trim()) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `${label} Host 不是 clean worktree：${root}`, 409);
  }
  return actual;
}

async function exportGitSnapshot(root: string, commit: string, output: string): Promise<void> {
  const temporary = mkdtempSync(path.join(tmpdir(), "pdh-host-snapshot-"));
...
async function extractAndVerifyPackage(assembly: ServiceProfileAssemblyPolicy): Promise<VerifiedPackage> {
  ...
  for (const entry of entries) {
    ...
    if (unique.has(safe)) throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `压缩包条目重复：${safe}`, 409);
    unique.add(safe);
  }
  ...
  try {
    ...
    if (files.some((file) => FORBIDDEN_ARCHIVE_PATH.test(file))) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "Pah 业务模块包不得包含 COOL Hook 入口", 409);
    }
    ...
    if (
      ...
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 kind/installerCompatibility 不允许由 Pah 声明式装配", 409);
    }
    if (metadata.source?.dirty !== false || !/^[a-f0-9]{40}$/.test(metadata.source?.commit ?? "")) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包必须来自 clean、精确 commit", 409);
    }
    ...
    if (
      ...
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 manifest/module/version 身份不一致", 409);
    }
    ...
    if (integrity.formatVersion !== 1 || integrity.algorithm !== "sha256" || !Array.isArray(integrity.files)) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 integrity.json 格式不合法", 409);
    }
    const expectedFiles = files.filter((file) => file !== integrityPath);
    if (integrity.files.length !== expectedFiles.length) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 integrity 文件数量不匹配", 409);
    }
    const declared = new Map<string, { size: number; sha256: string }>();
    for (const raw of integrity.files) {
      if (!raw || typeof raw !== "object" || Array.isArray(raw)) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 integrity 条目不合法", 409);
      }
      ...
      if (
        ...
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `业务包 integrity 条目不合法：${relative}`, 409);
      }
    ...
    for (const relative of expectedFiles) {
      ...
      if (!item || statSync(absolute).size !== item.size || sha256File(absolute) !== item.sha256) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `业务包文件完整性不匹配：${relative}`, 409);
      }
    }
    if (!Array.isArray(metadata.payloads) || metadata.payloads.length === 0) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包缺少 payloads", 409);
    }
    return {
      extractedRoot,
      metadata,
  ...
  const visit = (directory: string) => {
    for (const entry of readdirSync(directory, { withFileTypes: true })) {
      ...
      if (FORBIDDEN_PROTOCOL.test(text)) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `依赖配置包含本地协议：${absolute}`, 409);
      }
      if (entry.name === "package.json") {
        ...
        if (
          ...
          throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `依赖配置包含 override/resolution：${absolute}`, 409);
        }
        for (const section of dependencySections(packageJson)) {
          for (const specifier of Object.values(section)) {
            if (typeof specifier === "string" && (FORBIDDEN_PROTOCOL.test(specifier) || path.isAbsolute(specifier))) {
              throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `依赖配置包含本地路径：${absolute}`, 409);
            }
          }
        }
      } else if (/^\s*(?:overrides|patchedDependencies):/mu.test(text)) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `pnpm 配置包含 override/patch：${absolute}`, 409);
      }
    }
  };
  visit(root);
...
function lockIntegrity(lockfile: string, item: ServiceProfileRegistryPackage): string {
  ...
  if (!match) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `lockfile 缺少 ${item.name}@${item.version} integrity`, 409);
  }
  return match[1]!.trim();
}
...
function verifyRegistryPackage(
  ...
  if (!existsSync(lockfile)) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `${item.serviceRole} 缺少 pnpm-lock.yaml`, 409);
  }
  ...
  if (!existsSync(manifestPath)) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `${item.name}@${item.version} 未在 assembly node_modules 解析`, 409);
  }
  ...
  if (installed.name !== item.name || installed.version !== item.version) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `${item.name} 实际解析身份不匹配`, 409);
  }
  return {
...
function compareVersion(left: string, right: string): number {
  ...
  if (!leftParts || !rightParts) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `无法校验非标准 semver：${left} / ${right}`, 409);
  }
...
function verifyHostPeers(
  ...
  for (const [dependency, range] of Object.entries(peerDependencies)) {
    ...
    if (!present) {
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `clean Host 缺少业务包 peerDependency：${dependency}`, 409);
    }
...
function verifyDatabasePolicy(
  ...
  if (!databaseService) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `Profile 缺少数据库责任服务：${policy.database.serviceRole}`, 409);
  }
  ...
  if (policy.database.forbiddenNames?.includes(actualDatabase)) {
    throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `数据库命中 Profile 禁止名单：${actualDatabase}`, 409);
  }
}
...
export class PdhProfileAssembly {
  inspect(definition: ServiceDefinition): ServiceProfileEvidence | undefined {
    ...
    try {
      ...
      if (sha256File(policy.assembly!.packagePath) !== policy.assembly!.packageSha256) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 SHA-256 与 Profile 冻结值不一致", 409);
      }
      ...
      if (JSON.stringify(registry) !== JSON.stringify(document.registryPackages)) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "assembly Registry 依赖证据已变化", 409);
      }
  ...
  async prepare(
    ...
    if (existsSync(assembly.outputRoot)) {
      if (!existsSync(evidencePath)) {
        throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `装配目录已存在但缺少 evidence，拒绝覆盖：${assembly.outputRoot}`, 409);
      }
      const document = readJson(evidencePath) as unknown as AssemblyEvidenceDocument;
      this.#assertEvidenceIdentity(definition, document);
    ...
    try {
      ...
      for (const payload of verifiedPackage.metadata.payloads!) {
        if (payload.runtime !== "node" && payload.runtime !== "vue") {
          throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `业务包 payload runtime 不支持：${String(payload.runtime)}`, 409);
        }
        ...
        if (!isInside(verifiedPackage.extractedRoot, source) || !isInside(path.join(assembly.outputRoot, payload.runtime), target)) {
          throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "业务包 payload 路径逃逸", 409);
        }
        if (!existsSync(source) || existsSync(target)) {
          throw new DevHubError("PROFILE_PREFLIGHT_FAILED", `业务包 payload 缺失或目标冲突：${target}`, 409);
        }
        walkFiles(source);
  ...
  #assertEvidenceIdentity(definition: ServiceDefinition, document: AssemblyEvidenceDocument): void {
    ...
    if (
      ...
      throw new DevHubError("PROFILE_PREFLIGHT_FAILED", "assembly evidence 与当前 Profile 冻结配置不一致", 409);
    }
  }
}
