...
export interface PdhProjectImportChange {
  readonly project: LocalNodeProject;
  readonly definition: ServiceDefinition;
}

export interface PdhProjectImportPlan {
  readonly projects: readonly LocalNodeProject[];
  readonly added: readonly PdhProjectImportChange[];
  readonly updated: readonly PdhProjectImportChange[];
}

function configError(message: string, statusCode = 500): never {
  throw new DevHubError("INVALID_LOCAL_PROJECT", message, statusCode);
}
...
/** 管理 Dev Hub 本机私有的 Node.js 项目清单。 */
export class PdhProjectConfigStore {
  ...
  add(
    ...
    if (!candidate.scripts.includes(script)) {
      throw new DevHubError("SCRIPT_NOT_FOUND", `package.json 中不存在 script：${script}`, 400);
    }
  ...
  update(
    ...
    if (index < 0) throw new DevHubError("PROJECT_NOT_FOUND", `未知本机项目：${projectId}`, 404);
    ...
    if (this.#projects.some((project) => project.id !== projectId && project.directory === candidate.directory)) {
      throw new DevHubError("PROJECT_ALREADY_CONFIGURED", "该本地项目已经加入启动列表", 409);
    }
    const script = stringValue(scriptInput, "script");
    if (!candidate.scripts.includes(script)) {
      throw new DevHubError("SCRIPT_NOT_FOUND", `package.json 中不存在 script：${script}`, 400);
  ...
  prepareImport(value: unknown, reservedServiceIds: ReadonlySet<string>): PdhProjectImportPlan {
    const root = objectValue(value, "导入配置");
    if (root.format !== "phoenix-dev-hub-projects" || root.version !== 1 || !Array.isArray(root.projects)) {
      return configError("导入配置必须使用 phoenix-dev-hub-projects version=1 格式", 400);
    }
    ...
    const added: PdhProjectImportChange[] = [];
    const updated: PdhProjectImportChange[] = [];
  ...
  commitImport(plan: PdhProjectImportPlan): void {
