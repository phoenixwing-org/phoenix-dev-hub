...
export interface PdhPersistedOwnership {
...
interface PdhOwnershipFile {
  readonly version: 1;
  readonly records: readonly PdhPersistedOwnership[];
}
...
function ownershipRecord(value: unknown): PdhPersistedOwnership | undefined {
  if (!value || typeof value !== "object" || Array.isArray(value)) return undefined;
  const item = value as Record<string, unknown>;
...
export function pdhServiceDefinitionIdentity(definition: ServiceDefinition): string {
...
export interface PdhServiceOwnershipPersistence {
  entries(): readonly PdhPersistedOwnership[];
  put(record: PdhPersistedOwnership): void;
  delete(serviceId: string, ownershipId?: string): void;
}

export class PdhMemoryServiceOwnershipStore implements PdhServiceOwnershipPersistence {
  readonly #records = new Map<string, PdhPersistedOwnership>();

  constructor(records: readonly PdhPersistedOwnership[] = []) {
...
export class PdhServiceOwnershipStore implements PdhServiceOwnershipPersistence {
  ...
  #load(): void {
    ...
    try {
      const document = JSON.parse(readFileSync(this.#file, "utf8")) as Partial<PdhOwnershipFile>;
  ...
  #save(): void {
    ...
    const document: PdhOwnershipFile = { version: 1, records: this.entries() };
    try {
