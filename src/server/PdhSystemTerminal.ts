...
export function pdhSystemTerminalCapability(
...
export function pdhSystemTerminalLaunchSpecs(
  directory: string,
...
export class PdhSystemTerminal {
  constructor(
  ...
  capability(): SystemTerminalCapability {
    return pdhSystemTerminalCapability(this.platform, this.environment);
  }

  async open(serviceId: string, directory: string): Promise<OpenSystemTerminalResponse> {
    const capability = this.capability();
    if (!capability.available) {
      throw new DevHubError("SYSTEM_TERMINAL_UNAVAILABLE", capability.reason ?? "系统终端不可用", 409);
    }
    ...
    if (!directoryExists) {
      throw new DevHubError("SERVICE_CWD_NOT_FOUND", `服务目录不存在：${directory}`, 409);
    }

    const specs = pdhSystemTerminalLaunchSpecs(directory, this.platform as SupportedPlatform);
    ...
    throw new DevHubError("SYSTEM_TERMINAL_OPEN_FAILED", `无法打开系统终端：${message}`, 500);
  }
}
