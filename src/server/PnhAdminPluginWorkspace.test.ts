...
function fixture(): {
  ...
  web: string;
  node: string;
  workspace: PnhAdminPluginWorkspace;
} {
  const root = mkdtempSync(path.join(os.tmpdir(), "pnh-admin-plugin-"));
  roots.push(root);
  ...
  return {
    ...
    web,
    node,
    workspace: new PnhAdminPluginWorkspace(hub, {
      adminWebRoot: web,
      adminNodeRoot: node,
    }),
  };
...
describe("Admin 插件开发工作区", () => {
  ...
  it("只管理双端链接与 Git exclude，不执行 Admin Node 内部实体脚本", () => {
    ...
    const workspace = new PnhAdminPluginWorkspace(current.hub, {
  ...
  it("拒绝覆盖实体目录、外来版本链接和未开发卸载的列表移除", () => {
    ...
    expect(() => current.workspace.remove(second.registration.id)).toThrow("不是可安全清理的受控链接");
    expect(current.workspace.remove(
      second.registration.id,
      `cleanup-nonmounted:${second.registration.id}`,
    )).toEqual(second.registration);
  ...
  it("旧目录删除后只清理精确受控的悬空链接，再移除登记", () => {
    ...
    expect(current.workspace.status(added.registration.id)).toMatchObject({
      sourceState: "unavailable",
      mountState: "mounted",
    });
    expect(current.workspace.remove(added.registration.id)).toEqual(added.registration);
    expect(current.workspace.catalog().plugins).toHaveLength(0);
    for (const mount of mounted.mounts) {
      expect(existsSync(mount.target)).toBe(false);
      expect(readFileSync(mount.excludePath, "utf8")).not.toContain(mount.excludePattern);
    }
  });

  it("旧目录删除后默认拒绝外来 Host 链接，明确确认后清理链接并移除登记", () => {
    const current = fixture();
    const oldProduct = pluginFixture(current.root, "0.1.0");
    const added = current.workspace.add(oldProduct);
    const mounted = current.workspace.mount(added.registration.id);
    ...
    directoryLink(foreignSource, mounted.mounts[0]!.target);
    rmSync(oldProduct, { recursive: true, force: true });

    expect(() => current.workspace.remove(added.registration.id)).toThrow("不是可安全清理的受控链接");
    expect(current.workspace.catalog().plugins).toHaveLength(1);
    expect(realpathSync(mounted.mounts[0]!.target)).toBe(realpathSync(foreignSource));
    expect(current.workspace.remove(
      added.registration.id,
      `cleanup-nonmounted:${added.registration.id}`,
    )).toEqual(added.registration);
    expect(current.workspace.catalog().plugins).toHaveLength(0);
  ...
  it("早期登记缺少 moduleId 时只凭受控旧链接与 marker 恢复并修改目录", () => {
    ...
    const legacy = new PnhAdminPluginWorkspace(current.hub, {
      adminWebRoot: current.web,
      adminNodeRoot: current.node,
    });
  ...
  it("早期登记缺少身份且没有任何受控旧链接时拒绝猜测 moduleId", () => {
    ...
    mkdirSync(mounted.mounts[0]!.target, { recursive: true });
    mkdirSync(mounted.mounts[1]!.target, { recursive: true });
    expect(() => legacy.remove(added.registration.id)).toThrow("明确确认强制清理");
    expect(legacy.remove(
      added.registration.id,
      `cleanup-nonmounted:${added.registration.id}`,
    )).toMatchObject({
      id: added.registration.id,
      productRoot: added.registration.productRoot,
      manifestPath: added.registration.manifestPath,
    });
    expect(legacy.catalog().plugins).toHaveLength(0);
    expect(existsSync(mounted.mounts[0]!.target)).toBe(true);
    expect(existsSync(mounted.mounts[1]!.target)).toBe(true);
  });
  ...
  it("登记保存失败时恢复原链接，并明确报告自动回滚登记无法复写", () => {
    ...
    const reloaded = new PnhAdminPluginWorkspace(current.hub, {
      adminWebRoot: current.web,
      adminNodeRoot: current.node,
    });
