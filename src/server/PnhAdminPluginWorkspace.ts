...
function fail(code: string, message: string, statusCode = 400, details?: unknown): never {
  throw new HubError(code, message, statusCode, details);
}
...
export class PnhAdminPluginWorkspace {
  ...
  constructor(
    ...
    this.#projectRoot = realDirectory(projectRoot, "Hub 根目录");
  ...
  remove(id: string, confirmation?: string): AdminPluginRegistration {
    const current = this.status(id);
    if (current.mountState === "unmounted") {
      this.#forgetRegistration(id);
      return current.registration;
    }
    if (current.sourceState === "unavailable" || current.mountState !== "mounted") {
      const force = confirmation === `cleanup-nonmounted:${id}`;
      this.#cleanupNonMountedRegistration(id, current, force);
      return current.registration;
    }
  ...
  /**
   * 非完整开发挂载状态默认只接受可证明安全的受控链接。
   * 用户明确确认强制清理时，删除目标槽位上的任何 symlink 并移除登记；
   * 实体目录与普通文件始终保留。来源可用且完整 mounted 的状态不会进入此方法。
   */
  #cleanupNonMountedRegistration(id: string, current: AdminPluginStatus, force: boolean): void {
    const moduleId = current.registration.moduleId ?? (force ? this.#legacyCleanupModuleId(current) : undefined);
    if (!moduleId) {
      if (force) {
        this.#forgetRegistration(id);
        return;
      ...
      return fail(
        "ADMIN_PLUGIN_CLEANUP_CONFIRM_REQUIRED",
        "插件不是完整开发挂载状态，但登记没有足够的模块身份或 Host 明细；请明确确认强制清理",
        409,
        current.mounts,
      );
    }
    const targets = this.#registrationMountTargets(current.registration, moduleId);
    const originals = targets.map((target) => {
      const mount = this.#inspectMount(moduleId, target);
      if (
        !force
        && (
          !["missing", "mounted"].includes(mount.linkState)
          || !["missing", "managed"].includes(mount.excludeState)
        )
      ) {
        return fail(
          "ADMIN_PLUGIN_UNAVAILABLE_CLEANUP_CONFLICT",
          `插件源目录不可用，且 Host 目标不是可安全清理的受控链接：${target.target}`,
          409,
          current.mounts,
        );
      }
      ...
      return { target, mount, link, excludePath, excludeContent: readOptionalFile(excludePath) };
    });
    ...
    for (const entry of originals) {
      ...
      if (currentLink.state !== entry.link.state || currentLink.value !== entry.link.value) {
        return fail(
          "ADMIN_PLUGIN_UNAVAILABLE_CLEANUP_RACE",
          `预检后 Host 链接发生变化，已停止清理：${entry.target.target}`,
          409,
        );
      }
      if (readOptionalFile(entry.excludePath) !== entry.excludeContent) {
        return fail(
          "ADMIN_PLUGIN_UNAVAILABLE_CLEANUP_RACE",
          `预检后 Git exclude 发生变化，已停止清理：${entry.excludePath}`,
          409,
        );
      }
    }
    ...
    const removedLinks: typeof originals = [];
    try {
      for (const entry of originals) {
        if (entry.link.state === "link") {
          unlinkSync(entry.target.target);
          removedLinks.push(entry);
        }
        try {
          this.#writeExclude(moduleId, entry.target, entry.excludePath, entry.excludeContent, false);
        } catch (error) {
      ...
      for (const entry of removedLinks.reverse()) {
        try {
          if (this.#rawLinkState(entry.target.target).state !== "missing") {
      ...
      for (const entry of originals) {
        try { writeAtomic(entry.excludePath, entry.excludeContent); } catch (rollbackError) {
          rollbackErrors.push(rollbackError instanceof Error ? rollbackError.message : String(rollbackError));
        }
      }
      ...
      if (rollbackErrors.length > 0) {
        return fail(
          "ADMIN_PLUGIN_UNAVAILABLE_CLEANUP_ROLLBACK_FAILED",
          "清理失效插件登记失败，且自动回滚未完整完成；请停止 Admin Host 后检查两端开发链接、Git exclude 与本机登记",
          500,
  ...
  #legacyCleanupModuleId(current: AdminPluginStatus): string | undefined {
    const moduleIds = new Set(
      (current.recentOperation?.changes ?? [])
        .filter((change) => ["created-link", "removed-link", "replaced-link", "claimed-link"].includes(change.action))
        .map((change) => path.basename(change.path))
        .filter((value) => MODULE_ID_PATTERN.test(value)),
    );
    return moduleIds.size === 1 ? [...moduleIds][0] : undefined;
  }

  #forgetRegistration(id: string): void {
  ...
  status(id: string): AdminPluginStatus {
    ...
    try {
      ...
      const sourceError = error instanceof HubError
