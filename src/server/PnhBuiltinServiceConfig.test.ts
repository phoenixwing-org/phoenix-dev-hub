...
function fixture(): { root: string; baseline: ServiceDefinition } {
  const root = mkdtempSync(path.join(os.tmpdir(), "pnh-builtin-config-"));
  roots.push(root);
...
describe("PnhBuiltinServiceConfigStore", () => {
  it("本机覆盖的工作目录失效时保留条目和配置错误，不阻断 Store/Hub 加载", () => {
    ...
    const store = new PnhBuiltinServiceConfigStore(root, {
      source,
  ...
  it("区分仓库基线、本机覆盖和移除，并可重置恢复", () => {
    const { root, baseline } = fixture();
    const store = new PnhBuiltinServiceConfigStore(root, [baseline]);
    ...
    const reloaded = new PnhBuiltinServiceConfigStore(root, [baseline]);
