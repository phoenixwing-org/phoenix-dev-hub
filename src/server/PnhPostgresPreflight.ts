...
interface PnhPostgresConnectionOptions {
...
interface PnhPostgresSession {
  query(text: string, values?: readonly unknown[]): Promise<{ readonly rows: readonly Record<string, unknown>[] }>;
  end(): Promise<void>;
}

export type PnhPostgresConnector = (
  options: PnhPostgresConnectionOptions,
) => Promise<PnhPostgresSession>;

export interface PnhProfileDatabasePreflight {
  inspect(
...
const defaultConnector: PnhPostgresConnector = async (options) => {
  const client = new Client({
    ...options,
    application_name: "phoenix-hub-release-preflight",
 ...
 */
export class PnhPostgresPreflight implements PnhProfileDatabasePreflight {
  readonly #connector: PnhPostgresConnector;
  readonly #processEnv: NodeJS.ProcessEnv;
  readonly #cache = new Map<string, CachedEvidence>();
  readonly #creating = new Set<string>();

  constructor(
    connector: PnhPostgresConnector = defaultConnector,
  ...
  async createIsolated(
    ...
    if (preflight.host !== "127.0.0.1" && preflight.host !== "::1" && preflight.host !== "localhost") {
      throw new HubError("PROFILE_DATABASE_CREATE_DENIED", "只允许在本机 PostgreSQL 创建验收数据库", 403);
    }
    if (database.name === preflight.maintenanceDatabase || !creation.allowedDatabaseNames.includes(database.name)) {
      throw new HubError("PROFILE_DATABASE_CREATE_DENIED", "目标数据库未进入精确验收 allowlist", 403);
    }
    const expectedConfirmation = `create-release-validation-database:${database.name}`;
    if (confirmation !== expectedConfirmation) {
      throw new HubError("CONFIRMATION_REQUIRED", `确认文本必须为 ${expectedConfirmation}`, 409);
    }
    const key = profileKey(definition);
    if (this.#creating.has(key)) {
      throw new HubError("PROFILE_DATABASE_CREATE_BUSY", "该 Profile 正在创建隔离数据库", 409);
    }
    ...
    try {
      ...
      if (!before || before.state === "unavailable" || before.state === "not-configured") {
        throw new HubError("PROFILE_DATABASE_PREFLIGHT_FAILED", before?.message ?? "数据库 preflight 不可用", 503);
      }
      if (before.exists !== false || before.state !== "missing") {
        throw new HubError("PROFILE_DATABASE_ALREADY_EXISTS", "目标验收数据库已存在，拒绝重复创建", 409, before);
      }
      ...
      if (!databaseService) {
        throw new HubError("PROFILE_DATABASE_PREFLIGHT_FAILED", "找不到数据库角色服务定义", 503);
      }
      ...
      let session: PnhPostgresSession | undefined;
      try {
      ...
      if (!after) {
        throw new HubError("PROFILE_DATABASE_CREATE_UNVERIFIED", "CREATE DATABASE 已返回，但复核未取得数据库证据", 500);
      }
      if (after.exists !== true || (after.state !== "ready" && after.state !== "uninitialized")) {
        throw new HubError("PROFILE_DATABASE_CREATE_UNVERIFIED", "CREATE DATABASE 已返回，但复核未确认目标数据库存在", 500);
      }
  ...
  #writeCreationEvidence(
    ...
    if (!outputRoot) {
      throw new HubError("PROFILE_DATABASE_EVIDENCE_FAILED", "发布验收装配目录缺失，无法记录建库责任", 500);
    }
    ...
    while (path.basename(runtimeRoot) !== ".runtime") {
      ...
      if (parent === runtimeRoot) {
        throw new HubError("PROFILE_DATABASE_EVIDENCE_FAILED", "装配目录不在 Hub .runtime 边界内", 500);
      }
      runtimeRoot = parent;
    }
